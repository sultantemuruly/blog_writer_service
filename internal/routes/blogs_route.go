...
func postBlog(db *pgx.Conn) http.HandlerFunc {
    return func(w http.ResponseWriter, r *http.Request) {
        ...
        if _, err := db.Exec(
            r.Context(),
            "INSERT INTO blogs (title, content, status) VALUES ($1, $2, $3)",
            req.Title, req.Content, *req.Status,
        ); err != nil {
            log.Println("Error inserting blog with status:", err)
            http.Error(w, "internal server error", http.StatusInternalServerError)
            return
        }
        log.Println("Blog inserted successfully with status")
        w.WriteHeader(http.StatusCreated)
        w.Write([]byte("Blog created successfully with status"))
    }
}
